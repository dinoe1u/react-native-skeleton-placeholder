...
/**
 * Global configuration that can be set once via SkeletonPlaceholderConfigProvider
 * and automatically applied to every SkeletonPlaceholder in the tree.
 * Any prop passed directly to SkeletonPlaceholder overrides the global value.
 */
export type SkeletonPlaceholderConfig = {
...
const DEFAULT_CONFIG: SkeletonPlaceholderConfig = {};

const SkeletonPlaceholderContext =
  React.createContext<SkeletonPlaceholderConfig>(DEFAULT_CONFIG);

type SkeletonPlaceholderConfigProviderProps = {
  config: SkeletonPlaceholderConfig;
  children: React.ReactNode;
};

export const SkeletonPlaceholderConfigProvider: React.FC<
  SkeletonPlaceholderConfigProviderProps
> = ({config, children}) => (
  <SkeletonPlaceholderContext.Provider value={config}>
    {children}
...
type SkeletonPlaceholderProps = {
   ...
   * Overrides the value set in SkeletonPlaceholderConfigProvider.
   */
  backgroundColor?: string;
  /**
...
const SkeletonPlaceholder: React.FC<SkeletonPlaceholderProps> & {
  ...
  shimmerWidth: shimmerWidthProp,
}) => {
  const globalConfig = React.useContext(SkeletonPlaceholderContext);

  // Local props take priority over global config, which takes priority over built-in defaults.
  const backgroundColor = backgroundColorProp ?? globalConfig.backgroundColor ?? '#E1E9EE';
  const highlightColor = highlightColorProp ?? globalConfig.highlightColor ?? '#F2F8FC';
  const speed = speedProp ?? globalConfig.speed ?? 800;
  const direction = directionProp ?? globalConfig.direction ?? 'right';
  const borderRadius = borderRadiusProp ?? globalConfig.borderRadius;
  const shimmerWidth = shimmerWidthProp ?? globalConfig.shimmerWidth;
